...
from wagtailmeili.testapp.models import MoviePage, MoviePageIndex, ReviewPage
...
def test_skip_models(meilisearch_backend, movies_index_page):
    ...
    # Test that non-skipped model works
    ...
    # Test that searching returns empty if model skipped by field value has been found
    jedi_results = meilisearch_backend.search("Return of the Jedi", MoviePage).get()
    assert len(jedi_results["hits"]) == 0
